...
"""
python scripts\llm_event_labeling.py --input data\fnspid\nasdaq_exteral_data.csv --output data\labels\silver_labels_test_upgraded.jsonl --api-key-file configs\keys.txt --use-article --market-data-file data\market_data.csv --max-workers 100 --max-samples 100 --inter-request-delay 3 --resume
"""
import argparse
import csv
import importlib
...
def infer_surprise(
    ...
    # fallback: use immediate price reaction as a proxy for surprise
    open_px = parse_numeric_field(get_raw_field(item, "open"))
    close_px = parse_numeric_field(get_raw_field(item, "close"))
    prev_close = parse_numeric_field(get_raw_field(item, "prev_close"))
    if open_px is None:
        open_px = parse_numeric_field(market_ctx.get("open"))
    if close_px is None:
        close_px = parse_numeric_field(market_ctx.get("close"))
    if prev_close is None:
        prev_close = parse_numeric_field(market_ctx.get("prev_close"))

    if open_px is not None and close_px is not None and open_px != 0:
        return clamp_float(
            (close_px - open_px) / abs(open_px), -1.0, 1.0
        ), "intraday_return"
    if close_px is not None and prev_close is not None and prev_close != 0:
        return clamp_float(
            (close_px - prev_close) / abs(prev_close), -1.0, 1.0
        ), "daily_return"
...
def infer_novelty(
    ...
    if encoder is None:
        ...
        for prior_text in prior_texts:
            ...
            if sim > best_sim:
                best_sim = sim
        novelty = clamp_float(1.0 - best_sim, 0.0, 1.0)
        return novelty, f"1-max_jaccard:{best_sim:.3f}"
    ...
    try:
        ...
        query = embs[0]
        history = embs[1:]
        best_sim = float(np.max(history @ query)) if len(history) else 0.0
        novelty = clamp_float(1.0 - best_sim, 0.0, 1.0)
        return novelty, f"1-max_cosine:{best_sim:.3f}"
    except Exception:
        current_tokens = tokenize_for_similarity(current_text)
        if not current_tokens:
            return None, "no_text"
